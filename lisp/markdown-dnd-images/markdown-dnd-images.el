...
(defun dnd-copy-if-not-exists (file newfile)
  "Copy file to newfile if newfile no exists"
  (try-mkdir (file-name-directory newfile))
  (if (file-exists-p newfile) nil (copy-file file newfile)))

(defun dnd-insert-image-tag (text)
  (let* ((filename (file-name-nondirectory text))
        (sourcefile text)
        (dirfile (concat "../content/images/" filename)))
    (insert (format "![%s]({{site.baseurl}}/content/images/%s)" filename filename))
    (dnd-copy-if-not-exists sourcefile dirfile)))
  ...
  (file-name-as-directory
   (expand-file-name
    (concat "../content/images/"
            (replace-regexp-in-string "[/ ]+" "_" (file-name-sans-extension (file-name-nondirectory buffer-file-name)))))))
